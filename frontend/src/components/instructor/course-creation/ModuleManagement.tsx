...
const ModuleManagement: React.FC<ModuleManagementProps> = ({ course, onCourseUpdate }) => {
  ...
  const [showLessonAI, setShowLessonAI] = useState<{ moduleId: number | null; batchMode?: boolean }>({ moduleId: null, batchMode: false });
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // TODO: In production, upload to a proper file storage service (S3, Cloudinary, etc.)
  ...
  // AI handlers for module
  const handleModuleAIGenerate = async (data: any) => {
    if (data.modules && Array.isArray(data.modules)) {
      // Multiple modules generated - create them all
      try {
        for (const moduleData of data.modules) {
          const newModule = await CourseCreationService.createModule(course.id, {
          ...
          // If suggested lessons exist, create them too
          if (moduleData.suggested_lessons && Array.isArray(moduleData.suggested_lessons)) {
            for (const lessonData of moduleData.suggested_lessons) {
              await CourseCreationService.createLesson(course.id, newModule.id, {
                title: lessonData.title,
                description: lessonData.description || '',
                content_type: 'text',
                content_data: `# ${lessonData.title}\n\nContent to be developed...`,
      ...
      setModuleForm(prev => ({
        ...
        learning_objectives: data.learning_objectives || prev.learning_objectives
      }));
    }
  ...
  const handleLessonAIGenerate = async (data: any) => {
    ...
    if (data.lessons && Array.isArray(data.lessons)) {
        ...
        for (const lessonData of data.lessons) {
          ...
          setModules(prevModules => prevModules.map(mod => {
            if (mod.id === currentModule.id) {
              return {
                ...mod,
                lessons: [...(mod.lessons || []), createdLesson]
              };
            }
            return mod;
          }));
      ...
      setLessonForm(prev => ({
        ...prev,
        title: data.title || prev.title,
        content_data: data.content_data || prev.content_data,
        description: data.description || prev.description,
        learning_objectives: data.learning_objectives || prev.learning_objectives,
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setLessonForm(prev => ({
          ...
          learning_objectives: response.data.learning_objectives || prev.learning_objectives,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* AI Module Assistant */}
      {showModuleAI && (
        <div className="bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-lg border-2 border-purple-200 dark:border-purple-800 p-6">
          <div className="flex justify-between items-center mb-4">
            <h4 className="text-lg font-semibold text-purple-900 dark:text-purple-300">AI Module Assistant</h4>
            <button
              onClick={() => setShowModuleAI(false)}
          ...
          {/* Course Context Info */}
          <div className="mb-4 p-4 bg-white/50 dark:bg-slate-800/50 rounded-lg border border-purple-200 dark:border-purple-700">
            <h5 className="text-sm font-semibold text-purple-800 dark:text-purple-300 mb-2">
              📚 Course Context
            </h5>
            <div className="text-sm text-slate-700 dark:text-slate-300 space-y-1">
              ...
              {modules.length > 0 && (
                <div>
                  <p className="font-medium mt-2">Existing Modules ({modules.length}):</p>
                      ...
                      <li key={module.id} className="text-xs">
                        ...
                        {module.description && (
                          <span className="text-slate-500 dark:text-slate-400 ml-1">
                            - {module.description.substring(0, 60)}{module.description.length > 60 ? '...' : ''}
                          </span>
                        )}
                      </li>
                    ))}
                  ...
                  <p className="mt-2 text-xs text-purple-700 dark:text-purple-400 italic">
                    💡 The AI will generate Module {modules.length + 1} that builds upon these existing modules
                  </p>
                </div>
              )}
            </div>
          ...
          <AIContentGenerator
            ...
            courseId={course.id}
            batchMode={true}
            onGenerate={handleModuleAIGenerate}
            context={{
                ...
                <Draggable key={module.id} draggableId={`module-${module.id}`} index={index}>
                  {(provided, snapshot) => (
                    ...
                    >
                      ...
                      {/* AI Lesson Assistant */}
                      {showLessonAI.moduleId === module.id && (
                        <div className="p-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20">
                          <div className="flex justify-between items-center mb-4">
                            ...
                            <button
                              onClick={() => setShowLessonAI({ moduleId: null, batchMode: false })}
                              className="text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200"
                            >
                              ✕
                            </button>
                          ...
                          <AIContentGenerator
                            ...
                            courseId={course.id}
                            moduleId={module.id}
                            onGenerate={handleLessonAIGenerate}
                            batchMode={showLessonAI.batchMode}
                            context={{
                              courseTitle: course.title,
                                      ...
                                      {isTextContent && lessonForm.content_type !== 'mixed' && (
                                        <div className="flex gap-2">
                                          <AIContentButton
                                            lessonTitle={lessonForm.title}
                                            courseTitle={course.title}
                                            moduleTitle={modules.find(m => m.id === editingLesson.module_id)?.title}
                                            existingContent={lessonForm.content_data}
                                            isGenerating={isGeneratingContent}
                                            showDialog={showContentAI}
                                            onToggleDialog={() => setShowContentAI(!showContentAI)}

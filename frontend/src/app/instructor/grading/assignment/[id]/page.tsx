...
import { 
  FileText, User, Calendar, Clock, BookOpen, Download, ExternalLink, Star, 
  MessageSquare, History, AlertTriangle, CheckCircle, Eye, Award, Target, 
  Info, RefreshCw, AlertCircle, Save, Sparkles, Calculator, FileBarChart,
  ArrowLeft, ChevronDown, ChevronUp, Zap, Shield, TrendingUp, Hash,
  Mail, Bot, RotateCcw, Send, CircleCheck, XCircle
} from 'lucide-react';

// ═════════════════════════════════════════════════════
//  CONSTANTS
// ═════════════════════════════════════════════════════
const AUTO_REFRESH_INTERVAL = 20_000; // 20s polling for AI grade updates
const AUTO_SAVE_INTERVAL = 30_000;

// Quick grades as percentage of max points
...
const AssignmentGradingDetail = () => {
  ...
  const passingScore = submission?.assignment?.passing_score ?? 60;
  const maxPoints = submission?.assignment_points || 100;
  ...
  const isModificationRequested = submission?.assignment?.modification_requested;
  ...
  const fetchSubmission = useCallback(async (silent = false) => {
    ...
    try {
      const data = await GradingService.getAssignmentSubmissionDetail(submissionId, true);
      setSubmission(data);

      // Pre-fill form only on first load (not on silent refresh)
      if (!silent) {
  ...
  // ── Auto-refresh polling (for AI grading updates) ──
  useEffect(() => {
    if (!token || !submissionId) return;
    const interval = setInterval(() => fetchSubmission(true), AUTO_REFRESH_INTERVAL);
    // Also refresh on tab visibility
    const handleVisibility = () => {
      if (document.visibilityState === 'visible') fetchSubmission(true);
    };
    ...
    try {
      ...
      if (hasExistingGrade) {
        await GradingService.updateAssignmentGrade(submissionId, {
          grade: gradeValue, feedback, max_resubmissions: resubLimit
        });
      } else {
      ...
      // Clear draft
      localStorage.removeItem(`grading_draft_assignment_${submissionId}`);

      // Show success toast then redirect
      setSuccessMsg(`Grade ${hasExistingGrade ? 'updated' : 'submitted'} successfully! Redirecting...`);
      setTimeout(() => router.push('/instructor/grading?status=graded'), 1500);
    } catch (err: any) {
  ...
  const applyQuickGrade = (pct: number) => {
    const val = Math.round((pct / 100) * maxPoints * 10) / 10;
    setGrade(val.toString());
  };
  ...
  const autoSaveDraft = useCallback(async () => {
    ...
    setAutoSaving(true);
    try {
      localStorage.setItem(`grading_draft_assignment_${submissionId}`, JSON.stringify({
  ...
  // Load draft on mount
  useEffect(() => {
    const saved = localStorage.getItem(`grading_draft_assignment_${submissionId}`);
    if (saved && !grade && !feedback) {
      try {
        const d = JSON.parse(saved);
        setGrade(d.grade || '');
        setFeedback(d.feedback || '');
        setRubricScores(d.rubricScores || {});
      } catch { /* ignore */ }
    }
  }, [submissionId]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    const iv = setInterval(autoSaveDraft, AUTO_SAVE_INTERVAL);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <div className="relative">
          ...
          <Award className="absolute inset-0 m-auto w-6 h-6 text-blue-500" />
        </div>
        <h3 className="mt-6 text-lg font-semibold text-slate-900 dark:text-white">Loading Submission</h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Fetching assignment details...</p>
      </div>
    );
  }
    ...
    return (
      <div className="text-center py-20 max-w-md mx-auto">
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-6">
          Could not find this submission or you lack permission to view it.
        </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                <span className="truncate max-w-[300px] sm:max-w-none">{submission.assignment_title}</span>
              </h1>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {submission.is_resubmission && (
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400">
                <RotateCcw className="w-3 h-3 mr-1" /> Resubmission #{submission.resubmission_count || 1}
              </span>
            )}
            {isModificationRequested && (
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/20 dark:text-amber-400">
                <AlertCircle className="w-3 h-3 mr-1" /> Modification Requested
              </span>
            )}
            {isAIGraded && (
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400">
                <Bot className="w-3 h-3 mr-1" /> AI Graded
              </span>
            )}
            {submission.days_late === 0 ? (
              ...
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400">
                <AlertTriangle className="w-3 h-3 mr-1" /> {submission.days_late}d Late
              </span>
            ) : null}
          </div>
        </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {submission.assignment?.modification_request_reason && (
                <div className="mt-1.5 text-sm text-amber-700 dark:text-amber-400">
                  <MarkdownRenderer
                    content={submission.assignment.modification_request_reason}
                    variant="card"
                    className="prose-amber text-sm"
                  />
                </div>
              )}
              <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-amber-600 dark:text-amber-500">
                {submission.assignment?.modification_request_at && (
                  <span className="flex items-center gap-1">
                    <Clock className="w-3 h-3" />
                    {new Date(submission.assignment.modification_request_at).toLocaleString()}
                  </span>
                )}
                {submission.assignment?.can_resubmit && (
                  <span className="flex items-center gap-1">
                    <CheckCircle className="w-3 h-3" /> Student can resubmit
                  </span>
                )}
              </div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-3 text-center">
              ...
              <p className="text-xl font-bold text-blue-600 dark:text-blue-400">{passingScore}%</p>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-3 text-center">
          ...
          {/* ── Assignment Description (Collapsible) ── */}
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              >
                <span className="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-300">
                  <Info className="w-4 h-4 text-slate-500" /> Assignment Description
                </span>
              ...
              {showDescription && (
                <div className="px-4 pb-4 border-t border-slate-100 dark:border-slate-700">
                  <div className="mt-3 bg-slate-50 dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                    <MarkdownRenderer content={submission.assignment.description} variant="card" className="prose-slate text-sm" />
                  </div>
                </div>
              )}
            </div>
          ...
          {submission.assignment?.instructions && (
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              >
                <span className="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-300">
                  <Target className="w-4 h-4 text-blue-500" /> Assignment Instructions
                </span>
              ...
              {showInstructions && (
                <div className="px-4 pb-4 border-t border-slate-100 dark:border-slate-700">
                  <div className="mt-3 bg-blue-50 dark:bg-blue-900/10 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
                    <MarkdownRenderer content={submission.assignment.instructions} variant="card" className="prose-blue text-sm" />
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-500 to-teal-600 px-4 py-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {submission.files_count !== undefined && submission.files_count > 0 && (
                    <span className="bg-white/20 text-white px-2 py-0.5 rounded-full text-xs font-medium">
                      {submission.files_count} file{submission.files_count > 1 ? 's' : ''}
                    </span>
                  )}
                  {submission.submission_status?.is_first === false && (
                ...
                {submission.due_date && (
                  <div className="flex items-center gap-2.5 text-sm">
                    ...
                    <div>
                      ...
                      <p className={`font-medium ${submission.days_late > 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-700 dark:text-slate-200'}`}>
                ...
                <div className="flex items-center gap-2.5 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-slate-700 dark:text-slate-200">
                      {submission.submission_status?.is_first ? 'First attempt' : `Resubmission #${submission.submission_status?.count || submission.resubmission_count || 1}`}
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-orange-700 dark:text-orange-400 mt-0.5">
                        {submission.submission_status.notes || submission.submission_notes}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {(submission.content || submission.text_content) && (
                <div className="mb-4">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-700 max-h-[400px] overflow-y-auto">
                    <MarkdownRenderer
                      content={submission.content || submission.text_content || ''}
                      variant="card"
              ...
              <SubmissionFileManager
                ...
                onDownloadSingle={async (fileId) => {
                  try {
                    ...
                    if (!file && fileId === 'main-file' && submission.file_url) {
                      file = { id: 'main-file', filename: submission.original_filename || 'Submission File', file_url: submission.file_url, file_size: 0, uploaded_at: submission.submitted_at };
                    }
                    if (file?.file_url) {
                ...
                onAddFileComment={(fileId, comment) => {
                  EnhancedFileService.addFileComment({
                    file_id: fileId, comment_text: comment, submission_id: submissionId, is_private: false
          ...
          {/* ── GRADING FORM ── */}
          <form id="grading-form" onSubmit={handleSubmitGrade}
                ...
                <div className="flex items-center gap-3">
                  {lastSaved && (
                    <span className="text-white/70 text-xs flex items-center gap-1">
                      <Save className="w-3 h-3" /> {lastSaved.toLocaleTimeString()}
                    </span>
                  )}
            ...
            <div className="p-4 space-y-5">
              ...
              <div className="flex flex-wrap gap-2 pb-4 border-b border-slate-200 dark:border-slate-700">
                <button type="button" onClick={() => setShowRubricPanel(!showRubricPanel)}
                  className={`flex items-center px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                    showRubricPanel ? 'bg-purple-600 text-white' : 'bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 hover:bg-purple-200 dark:hover:bg-purple-900/30'
                  }`}>
                  <FileBarChart className="w-3.5 h-3.5 mr-1.5" /> Rubric
                </button>
                <button type="button" onClick={() => setShowCommentBank(!showCommentBank)}
                  className={`flex items-center px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                    showCommentBank ? 'bg-blue-600 text-white' : 'bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 hover:bg-blue-200 dark:hover:bg-blue-900/30'
                  }`}>
                  <MessageSquare className="w-3.5 h-3.5 mr-1.5" /> Comments
                </button>
                <button type="button" onClick={autoSaveDraft} disabled={autoSaving}
                  className="flex items-center px-3 py-1.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 rounded-lg text-xs font-medium hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors disabled:opacity-50">
                  <Save className="w-3.5 h-3.5 mr-1.5" /> {autoSaving ? 'Saving...' : 'Save Draft'}
                </button>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                    Grade <span className="font-normal text-slate-400">/ {maxPoints} pts</span>
                  </label>
                  {grade && !isNaN(parseFloat(grade)) && (
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${colors.badge}`}>
                        {GradingService.getLetterGrade(pct)}
                      </span>
                    </div>
                  )}
                </div>

                {/* Grade input + quick grades */}
                <div className="flex gap-3 items-start">
                  <div className="flex-1 relative">
                    <input
                      ...
                      max={maxPoints}
                      step="0.1"
                      className="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-green-500 text-lg font-bold"
                      placeholder="0"
                    />
                  ...
                  <div className="flex flex-wrap gap-1">
                    {QUICK_GRADES.map(p => (
                      <button key={p} type="button" onClick={() => applyQuickGrade(p)}
                        className={`px-2 py-1.5 rounded-md text-xs font-semibold transition-colors border ${
                          Math.round(calculatePercentage()) === p
                            ? 'bg-green-600 text-white border-green-600'
                            : p >= passingScore
                            ? 'bg-green-50 dark:bg-green-900/10 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800 hover:bg-green-100'
                            : 'bg-red-50 dark:bg-red-900/10 text-red-700 dark:text-red-400 border-red-200 dark:border-red-800 hover:bg-red-100'
                        }`}>
                        {p}%
                      </button>
                    ))}
                ...
                {/* Grade progress bar with passing score marker */}
                {grade && !isNaN(parseFloat(grade)) && (
                  <div className="mt-3">
                    <div className="relative h-3 bg-slate-200 dark:bg-slate-700 rounded-full overflow-visible">
                      <div
                        className={`absolute top-0 left-0 h-full rounded-full transition-all duration-300 ${colors.bg}`}
                        style={{ width: `${Math.min(pct, 100)}%` }}
                      />
                      {/* Passing score marker */}
                      <div
                        className="absolute top-0 h-full w-0.5 bg-slate-900 dark:bg-white z-10"
                        style={{ left: `${passingScore}%` }}
                        title={`Passing: ${passingScore}%`}
                      />
                      <div
                        className="absolute -top-5 text-[9px] font-semibold text-slate-500 dark:text-slate-400 -translate-x-1/2"
                        style={{ left: `${passingScore}%` }}
                      >
                        Pass {passingScore}%
                      </div>
                    ...
                    {pct < passingScore && (
                      <p className="mt-2 text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1">
                        ...
                        Below passing threshold — modification will be auto-requested on submit
                      </p>
                    )}
                  </div>
                )}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-slate-400">
                    Used: {submission.submission_status?.count || submission.resubmission_count || 0} / {maxResubmissions || '3'}
                  </span>
                </div>
                <input
                  type="number"
                  ...
                  min="0" max="20" step="1"
                  className="w-28 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-green-500 text-sm"
                  placeholder="3"
                />
              </div>
              ...
              <div>
                    ...
                    {templates.length > 0 && (
                      <div className="relative">
                        <button type="button" onClick={() => setShowTemplates(!showTemplates)}
                          className="flex items-center px-2 py-1 text-xs bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 rounded-md hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">
                          <Eye className="w-3 h-3 mr-1" /> Templates
                        </button>
                        {showTemplates && (
                          <div className="absolute right-0 top-8 w-64 bg-white dark:bg-slate-700 rounded-lg shadow-xl border border-slate-200 dark:border-slate-600 z-20 max-h-48 overflow-y-auto">
                            {templates.map(t => (
                              <button key={t.id} type="button" onClick={() => applyTemplate(t)}
                                className="block w-full text-left px-3 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-600">
                ...
                <textarea
                  ...
                  rows={5}
                  className="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm"
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-4 border-t border-slate-200 dark:border-slate-700">
                <button type="submit" disabled={grading || !grade}
                  className="flex-1 flex items-center justify-center px-4 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-semibold text-sm shadow-sm">
                  {grading ? (
                    <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2" /> Submitting...</>
                  ) : (
                    <><Send className="w-4 h-4 mr-2" /> {hasExistingGrade ? 'Update Grade' : 'Submit Grade'}</>
                  )}
                </button>
                <button type="button" onClick={() => setShowModificationModal(true)}
                  className="flex items-center justify-center px-4 py-3 border border-amber-300 dark:border-amber-600 text-amber-700 dark:text-amber-400 rounded-lg hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors font-medium text-sm">
        ...
        <div className="space-y-4">
              ...
              <div className="p-4">
                ...
                <div className="space-y-2.5 text-sm">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-slate-600 dark:text-slate-300 truncate">{submission.student_info.email || 'N/A'}</span>
                  </div>
                  <div className="flex items-center gap-2.5">
                    <Hash className="w-4 h-4 text-slate-400" />
                    <span className="text-slate-600 dark:text-slate-300">ID: {submission.student_info.id}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 px-4 py-3">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <Sparkles className="w-4 h-4" /> AI Excel Grading
              </h3>
          ...
          {hasExistingGrade && submission.assignment_points && (
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              <div className="bg-gradient-to-r from-yellow-500 to-orange-500 px-4 py-3">
                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  <Star className="w-4 h-4" /> Current Grade
                </h3>
              </div>
              <div className="p-4">
                {/* Grade circle */}
                <div className="text-center">
                  {(() => {
                    const currentPct = GradingService.calculatePercentage(submission.grade!, submission.assignment_points);
                    ...
                    return (
                      <>
                        <div className={`inline-flex items-center justify-center w-20 h-20 rounded-full ${currentColors.bg} mb-3`}>
                          <div className="text-center">
                            <p className="text-lg font-bold text-white leading-tight">{submission.grade}</p>
                            <p className="text-[10px] text-white/80">/ {submission.assignment_points}</p>
                          </div>
                        </div>
                        <div className="space-y-1">
                          <p className="text-xl font-bold text-slate-900 dark:text-white">{currentPct.toFixed(1)}%</p>
                          <span className={`inline-block px-3 py-0.5 rounded-full text-sm font-semibold ${currentColors.badge}`}>
                            Grade: {letter}
                          </span>
                          {currentPct < passingScore && (
                            <p className="text-xs text-red-500 flex items-center justify-center gap-1 mt-1">
                              <XCircle className="w-3 h-3" /> Below passing ({passingScore}%)
                            </p>
                          )}
                        </div>
                      </>
                ...
                {isAIGraded && (
                  <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                    <div className="flex items-center justify-center gap-1.5 text-xs text-purple-600 dark:text-purple-400">
                      ...
                      <span className="font-medium">AI Auto-Graded</span>
                    </div>
                  </div>
                )}

                {submission.graded_at && (
                  <div className={`${isAIGraded ? 'mt-1' : 'mt-3 pt-3 border-t border-slate-200 dark:border-slate-700'} text-center`}>
                    <p className="text-[10px] text-slate-400 flex items-center justify-center gap-1">
                      <Clock className="w-3 h-3" /> {new Date(submission.graded_at).toLocaleString()}
                    </p>
                  </div>
                )}
                ...
                {submission.feedback && (
                  <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                    <div className="flex items-center gap-1.5 mb-2">
                      ...
                      <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Feedback</span>
                    </div>
          ...
          {submission.previous_attempts && submission.previous_attempts.length > 0 && (
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              <div className="bg-gradient-to-r from-slate-600 to-slate-700 px-4 py-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-white flex items-center gap-2">
                    <History className="w-4 h-4" /> Previous Attempts
                  </h3>
                      ...
                      {attempt.grade !== undefined ? (
                        <>
                          <span className="font-bold text-sm text-slate-900 dark:text-white">
                            {attempt.grade}/{maxPoints}
                          </span>
                          <p className="text-[10px] text-slate-400">
                            {GradingService.calculatePercentage(attempt.grade, maxPoints).toFixed(0)}%
                          </p>
                        </>
                      ) : (
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div className="p-4 space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">Points Possible</span>
                <span className="font-semibold text-slate-900 dark:text-white">{maxPoints}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">Passing Score</span>
                <span className="font-semibold text-blue-600 dark:text-blue-400">{passingScore}%</span>
              </div>
              {submission.due_date && (
                <div className="flex justify-between">
                  <span className="text-slate-500 dark:text-slate-400">Due Date</span>
                  <span className={`font-medium ${submission.days_late > 0 ? 'text-red-600' : 'text-slate-900 dark:text-white'}`}>
                    {new Date(submission.due_date).toLocaleDateString()}
                  </span>
                </div>
              )}
              ...
              {submission.assignment?.max_file_size_mb && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-slate-900 dark:text-white">{submission.assignment.max_file_size_mb}MB</span>
                </div>
              )}
              {submission.assignment?.max_resubmissions !== undefined && (
                <div className="flex justify-between">
                  <span className="text-slate-500 dark:text-slate-400">Max Resubmissions</span>
                  <span className="font-medium text-slate-900 dark:text-white">{submission.assignment.max_resubmissions}</span>
                </div>
              )}
              {submission.assignment?.allowed_file_types && (
                <div className="pt-2 border-t border-slate-200 dark:border-slate-700">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {submission.assignment.allowed_file_types.split(',').map((type: string, i: number) => (
                      <span key={i} className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-700 rounded text-[10px] font-medium text-slate-600 dark:text-slate-400">
                        {type.trim()}
                      </span>
                    ))}
                  </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              <div className="bg-gradient-to-r from-green-500 to-emerald-600 px-4 py-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="bg-white/20 text-white px-2 py-0.5 rounded-full text-xs font-medium">
                    {submission.assignment.rubric.total_points} pts
                  </span>
                </div>
              </div>
              <div className="p-3 max-h-80 overflow-y-auto space-y-2">
                {submission.assignment.rubric.criteria.map((criterion: any, idx: number) => (

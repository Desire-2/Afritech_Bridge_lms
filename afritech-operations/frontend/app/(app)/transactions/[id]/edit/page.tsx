...
export default function EditTransactionPage() {
  ...
  const id = params.id;
  const { data, error: loadErr, loading } = useFetch<any>(`/api/transactions/${id}`, [id]);
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      official_cost: t.official_cost ?? '',
      transaction_date: t.transaction_date || '',
      reference: t.reference || '',
      notes: t.notes || '',
    });
    if (t.service_id) {
      api('/api/services?per_page=200&active=true').then((d: any) => {
        const svc = (d.items || []).find((s: any) => s.id === t.service_id);
        setSelected(svc || null);
      });
    }
    if (t.client_id) {
      setClientSel({ id: t.client_id, full_name: t.client_name, client_number: t.client_number });
    }
  }, [data]);
  ...
  return (
    <div className="row">
      <div className="col-lg-8">
        <PageHeader title="Edit Transaction" subtitle={`Editing ${data?.transaction?.transaction_number || ''}`} />
        ...
        <form onSubmit={submit} className="card">
          <div className="card-body">
            ...
            <div className="d-flex gap-2">
              ...
              <button type="button" className="btn btn-outline-secondary" onClick={() => router.push(`/transactions/${id}`)}>Cancel</button>
            </div>
          </div>
        </form>
      </div>

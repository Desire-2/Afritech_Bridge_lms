...
from ..services.paypal_payment_service import PaypalPaymentService
...
@application_bp.route("/payment-methods", methods=["GET"])
def get_payment_methods():
    """
    Get recommended payment method based on user's country.
    Query params: country (required)
    Returns: {recommended_method, available_methods, is_mtn_region}
    """
    country = request.args.get("country", "")
    
    is_mtn_region = PaypalPaymentService.is_mtn_supported_country(country)
    recommended = PaypalPaymentService.get_recommended_payment_method(country)
    
    return jsonify({
        "country": country,
        "is_mtn_region": is_mtn_region,
        ...
        "available_methods": [
            {
                ...
                "available": is_mtn_region,
                "icon": "smartphone"
            },
            {
                "id": "paypal",
                "name": "PayPal",
                "description": "Pay securely with PayPal, credit or debit card",
                "available": True,
                "icon": "credit-card"
            }
        ],
        "mtn_supported_countries": list(PaypalPaymentService.MTN_SUPPORTED_COUNTRIES)
    }), 200
...
def initiate_payment():
    """
    Initiate payment before application submission.
    Body: {course_id, amount, currency, payment_method, phone_number, payer_name, email}
    Returns: {success, reference, approval_url (for paypal)}
    """
    ...
    if not course_id or not amount:
        return jsonify({"error": "course_id and amount are required"}), 400
    ...
    if payment_method == "mobile_money":
        ...
        payer_name = data.get("payer_name", "Course Applicant")
        
        if not phone_number:
            return jsonify({"error": "phone_number is required for mobile money payment"}), 400
        
        success, payment_ref, message = MtnPaymentService.process_mobile_money_payment({
            "amount": amount,
            "currency": currency,
        ...
        if success:
            return jsonify({
                "success": True,
                "reference": payment_ref or reference,
        ...
        success, approval_url, message = PaypalPaymentService.process_paypal_payment({
            "amount": amount,
            "currency": currency,
            "description": f"Course payment: {course.title}",
            "reference": reference,
        ...
        if success:
            ...
            return jsonify({
                "success": False,
                "error": message or "PayPal payment initiation failed"
            }), 400
    else:
...
def verify_payment():
    """
    Verify payment status (for mobile money after user approves).
    Body: {reference}
    ...
    success, status, message = MtnPaymentService.check_payment_status(reference)
...
@application_bp.route("/paypal/capture", methods=["POST"])
def capture_paypal_payment():
    """
    Capture a PayPal payment after user approval.
    Body: {order_id, application_id}
    """
    data = request.get_json() or {}
    ...
    success, capture_data, message = PaypalPaymentService.capture_order(order_id)
    
    if success:
        # Update application payment status if application_id provided
        if application_id:
            try:
                application = CourseApplication.query.get(application_id)
                if application:
                    application.payment_status = "completed"
                    application.payment_reference = order_id
                    db.session.commit()
            except Exception as e:
                logger.error(f"Failed to update application payment status: {e}")
...
@application_bp.route("/paypal/status/<order_id>", methods=["GET"])
def get_paypal_order_status(order_id):
    """Get the status of a PayPal order."""
    success, status, message = PaypalPaymentService.get_order_status(order_id)
    if success:
        return jsonify({
            "order_id": order_id,
            "status": status,
...
def apply_for_course():
    ...
    if course.enrollment_type == 'paid':
        ...
        # Auto-detect payment method based on country if not explicitly specified
        if not payment_method or payment_method == "auto":
            if PaypalPaymentService.is_mtn_supported_country(user_country):
                payment_method = "mobile_money"
            else:
                payment_method = "paypal"

        if not course.price or float(course.price) <= 0:
            return jsonify({"error": "Course price is not configured"}), 400

        if payment_method == "mobile_money":
            payment_phone = data.get("payment_phone_number") or data.get("phone")
            if not payment_phone:
                return jsonify({"error": "Mobile money number is required for payment"}), 400

            payment_success, payment_reference, payment_message = MtnPaymentService.process_mobile_money_payment({
                "amount": course.price,
                "currency": course.currency or "USD",
                "phone_number": payment_phone,
                "payer_name": data.get("payment_payer_name") or data.get("full_name"),
                "description": f"Course payment: {course.title}",
                "reference": str(uuid.uuid4()),
            })

            if not payment_success:
                return jsonify({"error": payment_message or "Payment initiation failed"}), 400

        elif payment_method == "paypal":
            payment_success, payment_approval_url, payment_message = PaypalPaymentService.process_paypal_payment({
                "amount": course.price,
                "currency": course.currency or "USD",
                "description": f"Course payment: {course.title}",
                "reference": str(uuid.uuid4()),
            ...
            if not payment_success:
                return jsonify({"error": payment_message or "PayPal payment initiation failed"}), 400
            
            payment_reference = payment_approval_url

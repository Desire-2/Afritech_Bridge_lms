...
export default function LoginForm() {
  ...
  const { isAuthenticated, login, isLoading, user, validateSession } = useAuth();
  ...
  // AGGRESSIVE authentication check - fail fast, show login form quickly
  useEffect(() => {
    let mounted = true;
    ...
    const checkAuthentication = () => {
      ...
      console.log('Quick auth check...', { isLoading, isAuthenticated, user: !!user });
      ...
      // If loading, wait just 1 second max
      if (isLoading) {
        console.log('Auth loading, waiting 1 second max...');
        setTimeout(() => {
          if (mounted && isAuthenticated && user && user.role) {
            console.log(`Auth loaded, redirecting ${user.role}`);
            const dashboardRoute = RolePermissions.getDashboardRoute(user.role);
            clearTimeout(forceTimeout);
            router.push(dashboardRoute);
          } else if (mounted) {
            console.log('Auth loaded but no user, showing login');
            setIsCheckingAuth(false);
            clearTimeout(forceTimeout);
          }
        }, 1000);
      } else {
        // Not loading and not authenticated, show login immediately
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error('Login timed out')), 15000); // 15 second timeout
      });
  ...
  // Show loading or checking authentication state - AGGRESSIVE TIMEOUT
  if (isCheckingAuth || isLoading) {
    const elapsed = authStartTime > 0 ? Math.floor((Date.now() - authStartTime) / 1000) : 0;
    
    // FORCE show login form after 3 seconds
    if (elapsed >= 3) {
      console.warn('FORCING login form display - elapsed 3+ seconds');
      setIsCheckingAuth(false);
      setAuthCheckTimeout(false);
      // Continue to render login form below
    } else {
      return (
          ...
          <div className="bg-white/5 backdrop-blur-sm p-8 rounded-2xl shadow-xl w-full max-w-md border border-white/10">
            <div className="text-center">
              ...
              <h3 className="text-xl font-semibold text-white mb-2">
                {elapsed >= 2 ? 'Loading Taking Too Long...' : 'Checking Authentication...'}
              </h3>
              <p className="text-slate-400 text-sm mb-4">
                {elapsed >= 2 ? 'Showing login form soon...' : 'Please wait while we verify your session'}
              </p>
              
              <div className="text-xs text-slate-500 mb-4">
                {elapsed}s elapsed {elapsed >= 2 && '(forcing login form)'}
              </div>
              ...
              {elapsed >= 1 && (
                <div className="mt-4">
                  <button
                    onClick={() => {
                      console.log('User manually requested login form');
                      setIsCheckingAuth(false);
                      setAuthCheckTimeout(false);
                    }}
              ...
              {isAuthenticated && user && elapsed >= 1 && (
                <div className="mt-2">
                  <button
                    onClick={() => {
                      const dashboardRoute = RolePermissions.getDashboardRoute(user.role);
                    ...
                    className="px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded text-sm transition-colors"
                  >
                    Go to Dashboard
                  </button>
                </div>
  ...
  return (
    <ClientOnly fallback={
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800">
        <div className="bg-white/5 backdrop-blur-sm p-8 rounded-2xl shadow-xl w-full max-w-md border border-white/10">
        {/* Show timeout warning if redirected due to timeout */}
        {timeoutReason && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-yellow-400/70 text-xs mt-1">
                  Your previous session took too long to load. Please log in again.
                </p>
              </div>
            </div>
          </div>
